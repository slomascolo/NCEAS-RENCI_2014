...
landuse_buffers_5000 <- subset(landuse_buffers, buffer == 5000 & YEAR == 1992)  
# just the 5000 buffer for 92

landuse_buffers_5000 <- landuse_buffers_5000 %>% select(RTENO, km2, land_cover) %>% 
...
head(landuse_buffers_5000)
range(rowSums(landuse_buffers_5000[2:8])) #hu

head(landuse_buffers_5000[, -1])

landuse_buffers_5000_nmds_prop <- landuse_buffers_5000[ , c("agriculture_prop", "barren_prop", 
                                                            "developed_prop", "forest_prop",
                                                            "grassland_shrub_prop", "water_prop", 
                                                            "wetlands_prop")]

landuse_buffers_5000_nmds <- landuse_buffers_5000[ , c("agriculture", "barren", "developed", 
...
landuse_buffers_5000$NMDS1 <- scores(nmds_veg)[,1]
landuse_buffers_5000$NMDS2 <- scores(nmds_veg)[,2]
try(landuse_buffers_5000$NMDS3 <- scores(nmds_veg)[,3])


clust_veg <- Mclust(landuse_buffers_5000_nmds)
#plot(clust_veg)
landuse_buffers_5000$classification <- as.factor(clust_veg$classification)

ordisurf(nmds_veg, landuse_buffers_5000$forest_prop, col = "forestgreen")
ordisurf(nmds_veg, landuse_buffers_5000$agriculture_prop, col = "purple3")
ordisurf(nmds_veg, landuse_buffers_5000$water_prop, col = "darkblue")

ggplot(landuse_buffers_5000, aes(x = NMDS1, y = NMDS2)) +
...
ggplot(landuse_buffers_5000, aes(x = NMDS1, y = NMDS2)) +
  geom_point(data = landuse_buffers_5000 %>% filter(agriculture_prop > 0.8), 
             size=3, aes(colour = "High ag"))+
  geom_point(data = landuse_buffers_5000 %>% filter(agriculture_prop < 0.8 & agriculture_prop > 0.5), 
             size=3, aes(colour = "Mod ag"))+
  geom_point(data = landuse_buffers_5000 %>% filter(agriculture_prop <= 0.5), 
