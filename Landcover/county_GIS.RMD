...
lc_all <- rbind(lc_county_2001, lc_county_2006, lc_county_2011)
write.csv(lc_all, file = "landcover_all.csv", row.names = FALSE)

#plots trends over time by landclass for a county
b <- ggplot(lc_all[lc_all$GEOID == 39115,], aes(x = year, y = total, group = GEOID)) + geom_point()
b + facet_wrap(~reclass, scales = "free") + geom_smooth(method = "lm")
