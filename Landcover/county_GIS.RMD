...
######################################
#Now combine landcover with pesticides
######################################
...
#do linear regression and extrapolate from 2001-2011 landcover trends
...
#Add agriculture landcover area to pesticide data
pesticides <- merge(pests, LandcoverEst[LandcoverEst$reclass == 6, ])
pesticides$kg_km2agri_high <- pesticides$KG_EPhigh / (pesticides$PredPix * 0.009)
pesticides$kg_km2agri_low <- pesticides$KG_EPlow / (pesticides$PredPix * 0.009)

write.csv(pesticides, file = "allpesticidesOH.csv")

```


Below, use the raster package to create new rasters of pesticide estimates from county shapefile.
This can then be used with site buffers to estimate pesticide use around BBS and butterfly surveys.

How to do it:
1. I have agriculture/pesticide use by county x year
2. I need landcover by buffer x county (and year), not sure if there's a way for this.

```{r rasterize pesticide shapefiles}
library(sp)
library(rgdal)
library(raster)
library(parallel)
...
pest <- read.csv("C:/Users/Tyson/REPO/NCEAS-RENCI_2014/Pesticides/allpesticidesOH.csv", header = TRUE)

#select which pesticides to use
pests <- c("ACETAMIPRID", "CLOTHIANIDIN", "FIPRONIL", "IMIDACLOPRID", "THIACLOPRID", "THIAMETHOXAM", "GLYPHOSATE", "ATRAZINE")
pest <- pest[which(pest$COMPOUND %in% pests), ]

#load raster for geo information
...
```{r landcover for survey buffers}
library(sp)
library(rgdal)
library(raster)
library(parallel)
...
pests <- c("ACETAMIPRID", "CLOTHIANIDIN", "FIPRONIL", "IMIDACLOPRID", "THIACLOPRID", "THIAMETHOXAM", "GLYPHOSATE", "ATRAZINE")
pest <- pest[which(pest$COMPOUND %in% pests), ]
pest <- pest[, c("GEOID", "YEAR", "COMPOUND", "kg_km2agri_high", "kg_km2agri_low")]
pest <- droplevels.data.frame(pest)

buffer_ag <- buffer_LC[buffer_LC$lc_class %in% c("81", "82"), ]
buffer_ag <- buffer_ag %>% group_by(BBS_route, GEOID, buffer) %>% summarise(km2agri = sum(pixels)*0.009) %>% data.frame()
bbs_pest <- merge(buffer_ag, pest)

bbs_pest$buffer <- as.character(bbs_pest$buffer)
bbs_pest_buff <- bbs_pest %>% filter(buffer == "5000") %>% group_by(BBS_route, YEAR, COMPOUND) %>% summarise(kg_total = sum(km2agri * kg_km2agri_high)) %>% data.frame()
names(bbs_pest_buff)[1] <- "BBSRTSL020"

mean_use <- bbs_pest_buff %>% group_by(BBSRTSL020, COMPOUND) %>% summarise(kg = mean(kg_total)) %>% data.frame()

#plot correlations between use of different compounds by bbs route buffer
library(reshape2)
mean_use_melt <- melt(mean_use)
mean_use_cast <- dcast(mean_use_melt, BBSRTSL020 ~ COMPOUND)
pairs.default(mean_use_cast[, c(2:9)])


#make color scale across all years to see change in intensity over time
compounds <- unique(pest$COMPOUND)
pest <- bbs_pest_buff[bbs_pest_buff$COMPOUND == compounds[8], ]


#select color palette and the number colors to represent on the map
colors <- brewer.pal(9, "YlOrRd")
#set breaks for the 10 colors
brks<-classIntervals(pest$kg_total, n=9, style="quantile")
brks<- brks$brks

par(mfrow=c(3,3), mar=c(1,1,1,1)+0.1, oma=c(1,1,1,1)+0.1)

for (i in min(pest$YEAR):max(pest$YEAR)){
  
  temp <- pest[pest$YEAR == i, ]
  to.map <- merge(bbs_buff_5000, temp, by = "BBSRTSL020")

  #plot the map
  plot(to.map, col=colors[findInterval(to.map$kg_total, brks,all.inside=TRUE)], axes=F)
