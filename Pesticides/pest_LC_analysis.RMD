...
.packages = c("rgdal", "raster","plyr", "dplyr", "ggplot2", "plotKML", "sp", "rgeos", "maptools", "stringr", "data.table", "snow")
...
#######
#How many compounds needed in this dataset? Trying to reduce size for github
test <- county.data %>%
  group_by(COMPOUND) %>%
...
temp <- county.data %>%
  group_by(COMPOUND) %>%
  summarise(nyear = length(unique(YEAR)), ncounty = length(unique(GEOID)))

temp2 <- county.data %>%
  group_by(COMPOUND) %>%
  mutate(num = n(), tots = sum(KG_EPhigh)) %>%
  filter(num >= 500)
...
test <- merge(lc, pest, allow.cartesian = TRUE)

###PROBLEM HERE
#Need to include zero's for use of different compounds.
#If compound isn't used in all counties the transect buffer intersects, the agri. pix total is off.
#High.kg probably still OK, but need zeros to get proper weighted average use rate across the buffer's agri pix total.

#this will aggregate counties into buffers
#high_kg_buff is total pesticide kg used within a buffer x year
pest_buff <- test[, `:=` (high.kg = pix * 0.0009 * high_kg_km2ag, 
                          low.kg = pix * 0.0009 * low_kg_km2ag)]
# pest_buff <- pest_buff[, `:=` (high_kg_buff = sum(high.kg),
#                                low_kg_buff = sum(low.kg),
#                                by = list(YEAR, RTENO, buffer, COMPOUND)]

# lc_ag_byRTENO <- lc[, Ag_pix := sum(pix), by = list(YEAR, RTENO, buffer)]

#NOT QUITE right, duplicates due to GEOID's not summarising, maybe use dplyr???

pest_buff1 <- pest_buff %>%
  group_by(YEAR, RTENO, buffer) %>% #figure out total agriculture land area for a RTENO across counties
  mutate(ag_km2 = sum(pix) * 0.0009) %>%
  group_by(YEAR, RTENO, buffer, COMPOUND, ag_km2) %>% #now figure out kg used for RTENO across counties
  summarise(high_kg_buff = sum(high.kg), low_kg_buff = sum(low.kg)) %>%
  mutate(meanbuff_kgkm2ag = high_kg_buff / ag_km2) #rate of use for a RTENO buffer 
  

# write.csv(pest_buff1, "Pesticides/pest_buff_overtime.csv", row.names = FALSE)
###FILE TOO BIG, SHARE VIA BOX instead
# what pesticides do we want for analysis?
# 
# #need updated data.table for fread using url from Box.com
# library(devtools)
...
# library(repmis)
# 
# #Try Karthik's rdrop2 packages next
...
#join pesticides with above data.table to get pesticide use in counties, then aggregate by buffers
pest <- fread("Pesticides/estimated_pesticides_OHcounty.csv", header = TRUE)
setkey(pest, GEOID, YEAR)
pest[,pix := NULL]
...
pest_buff1 <- pest_buff %>%
  group_by(YEAR, site, buffer) %>% #figure out total agriculture land area for a RTENO across counties
  mutate(ag_km2 = sum(pix) * 0.0009) %>%
  group_by(YEAR, site, buffer, COMPOUND, ag_km2) %>% #now figure out kg used for RTENO across counties
  summarise(high_kg_buff = sum(high.kg), low_kg_buff = sum(low.kg)) %>%
  mutate(meanbuff_kgkm2ag = high_kg_buff / ag_km2) #rate of use for a RTENO buffer 
  

#Too big for github 
write.csv(pest_buff1, "Pesticides/pest_bfly_buff_overtime.csv", row.names = FALSE)
...
```{r}
#is there a good way to compress pesticide data?

pest <- fread("C:/Users/Tyson/Desktop/Box Sync/Share Data/pest_buff_overtime.csv")
library(reshape2)
library(vegan)

#make list of pesticides for classification
test <- pest %>%
  filter(buffer == 10000) %>%
  group_by(COMPOUND) %>%
  summarise(yr_by_county_use = n(), mean_rate = mean(meanbuff_kgkm2ag), total_kg = sum(high_kg_buff))
...
top_pests <- pest %>%
  group_by(COMPOUND) %>%
  mutate(num = n()) %>%
  filter(num > 1000, meanbuff_kgkm2ag > 0.001) 

site_pest <- data.frame(cast(top_pests, RTENO + YEAR ~ COMPOUND, value = "meanbuff_kgkm2ag"))
site_pest[is.na(site_pest)] <- 0

site_pest[, -(1:2)] <- decostand(site_pest[, -(1:2)], 'standardize')
site_pest$RTENO <- as.factor(site_pest$RTENO)
