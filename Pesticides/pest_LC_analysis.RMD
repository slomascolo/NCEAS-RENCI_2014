...
lc_buffer_2001 <- buffer_LC2001 %>% 
  ...
  group_by(BBS_route, GEOID, buffer, reclass) %>% 
  summarise(total = sum(pixels)) %>% data.frame()
...
chg90s_1 <- buffer_LC1992 %>% filter(lc_class > 9) %>% group_by(BBS_route, GEOID, buffer, reclass92) %>% summarise(add_to_2001 = sum(pixels)) 
chg90s_2 <- buffer_LC1992 %>% filter(lc_class > 9) %>% group_by(BBS_route, GEOID, buffer, reclass01) %>% summarise(sub_from_2001 = sum(pixels))
names(chg90s_1)[4] <- "reclass"
names(chg90s_2)[4] <- "reclass"

chg90s <- merge(chg90s_1, chg90s_2, by = c("BBS_route", "GEOID", "buffer", "reclass"), all = TRUE)
chg90s_2001 <- merge(lc_buffer_2001, chg90s)
chg90s_2001$sub_from_2001 <- chg90s_2001$sub_from_2001 * -1
chg90s_2001$total_92est <- rowSums(chg90s_2001[, c("total", "add_to_2001", "sub_from_2001")], na.rm = TRUE)
lc_buffer_1992_backest <- chg90s_2001[, c("GEOID", "reclass", "BBS_route", "buffer", "total_92est")]
...
setkey(DT, GEOID, BBS_route)
lc_int_overtime <- DT[, LandcoverLine(data.frame(year = year, total = total)), by = list(BBS_route, GEOID, reclass, buffer)]
setnames(lc_int_overtime,"year","YEAR")
setkey(lc_int_overtime, BBS_route, YEAR)

# for some reason, data.table not collasping in same way as summarise in dplyr
lc_buff_overtime <- lc_int_overtime[, sum(pix, na.rm = TRUE), by = list(BBS_route, reclass, buffer, YEAR)]
...
pest <- fread("C:/Users/Tyson/Desktop/estimated_pesticides_OHcounty.csv", header = TRUE)
...
pest_buff <- pest_buff[, `:=` (high_kg_buff = sum(high.kg),
                               ...
                               by = list(YEAR, BBS_route, buffer, COMPOUND)]

#remove county GEOID and county-specific values, then remove duplicate rows
pest_buff2 <- pest_buff[, c("YEAR", "BBS_route", "buffer", "COMPOUND", "high_kg_buff", "low_kg_buff", "ag_pix_buff"), with = FALSE]
...
test <- LC_buffer_overtime %>%
  group_by(BBS_route, YEAR, buffer) %>%
...
write.csv(ohio_BBS, file = "fifty7_withRTENO.csv", row.names = FALSE)
...
#works, but error says 'from' value 2380 not present in x. Landcover analysis missing one route???
