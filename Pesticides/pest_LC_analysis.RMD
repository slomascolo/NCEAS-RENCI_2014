...
#butterfly count data (just raw) from previous ESA analysis
load("C:/Users/Tyson/Dropbox/Ohio/jan2014/populations/all.species.data.RData")
options(na.action = "na.fail") #why do I have this?

pest <- read.csv("pest_bflybuff_overtime.csv", header = TRUE)
names(pest) <- c("year", "site", "buffer", "compound", "high_kg_buff", "low_kg_buff", "ag_pix_buff")
...
lc <- read.csv("LC_bflybuff_overtime.csv", header = TRUE)
names(lc) <- c("site", "reclass", "buffer", "year", "total_pix", "km2")
lc_chg <- lc %>% group_by(site, reclass, buffer) %>% summarise(trend = )


pops <- all.species.env[, c("site", "year", "count", "Nt1", "species", "loglambda")]
pops$site <- as.character(pops$site)

pest$site <- as.character(pest$site)

dat <- merge(pops, pest)
#choose buffer, compound to model
buff <- 1000
comp <- unique(pest$compound)[7]
data <- dat[which(dat$buffer == buff & dat$compound == comp), ]
data$scale_year <- scale(data$year)
data$fact_year <- as.factor(data$year)
data$scale_high_kg_buff <- scale(data$high_kg_buff)
data <- data[complete.cases(data),]

mod.1 <- lme(loglambda ~ log(Nt1) + scale_year + scale_high_kg_buff, 
            random = list(species=~scale_high_kg_buff, species=~year), data = data)

sp.re <- random.effects(mod.1, 1)
plot(sp.re)
```
