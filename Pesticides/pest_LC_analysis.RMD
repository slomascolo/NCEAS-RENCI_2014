...
.packages = c("rgdal", "raster", "plyr", "dplyr", "ggplot2", "plotKML", "sp", "rgeos", "maptools", "stringr", "data.table")
...
temp2 <- county.data %>%
  group_by(COMPOUND) %>%
  summarise(num = n(), tots = sum(KG_EPhigh)) %>%
  arrange(num) %>%
  data.frame()
...
RERUN STARTING HERE FOR BUFFERS AROUND BBS RTEs
MIGHT BE MORE RECENT ONES IN NEW FILE I DOWNLOADED
...
bbs_routes <- readOGR(dsn = "C:/Users/Tyson/Desktop/Box Sync/Neonicotenoids/Final GIS layers", layer = bbs.shp)
