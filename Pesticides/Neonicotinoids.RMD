...
pest <- OH.neonico[OH.neonico$COMPOUND == compounds[5], ]
...
proj4string(r) <- CRS("+init=epsg:2163")
...
####HOLD OFF ON 2011 for now, buffer go outside OH and merging rasters sucks
###get 2011 NLCD data from multiple rasters and merge
setwd("C:/Users/Tyson/Desktop/NLCD GIS/2011/land_use_land_cover")

test <- list(raster("nlcd_oh_utm17.tif"), raster("pa_proj2.tif"))

files <- list.files()
tif <- files[grep(".tif", files)]
tif <- tif[-grep(".dbf", tif)]

tif.list <- list()
tifproj <- tif.list[[1]]
proj4string(tifproj) <- CRS("+init=epsg:5070")

for (i in 1:length(tif)){
  tif.list[[i]] <- reproject(raster(tif[i]), CRS("+init=epsg:5070"))
}

#merge or mosaic, overlapping but merge did fine I think
test$filename <- 'OH_NLCD2011.tif'
test$overwrite <- TRUE
# test$tolerance  <- 1
OH_LC2011 <- do.call(merge, test)
