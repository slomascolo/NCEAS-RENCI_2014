...
##The model, varying intercept and slope per RTENO and varying intercept for YEAR using Wishart distribution
W <- diag (K)
birdW.data <- c(bird.data,list(W=W))
birdW.inits <- function(){
    list (B.raw=array(rnorm(K*J), c(J,K)), mu.raw=rnorm(K),
          sigma.y=runif(1),
