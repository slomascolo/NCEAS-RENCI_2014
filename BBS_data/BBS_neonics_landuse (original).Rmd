...
BBS_ln_scale <- BBS_ln %>%
  ...
  mutate(ag_scaled = as.numeric(scale(agriculture)), 
         forest_scaled = as.numeric(scale(high_kg_buff)))

names(BBS_ln_scale)
BBS_ln_scale <- BBS_ln_scale %>%
  group_by(COMPOUND, buffer) %>%
  mutate(high_kg_buff_scaled = as.numeric(scale(high_kg_buff)))
names(BBS_ln_scale) <- toupper(names(BBS_ln_scale))

BBS_ln_scale$NEONIC <- ifelse(BBS_ln_scale$COMPOUND %in% c("GLYPHOSATE", "ATRAZINE"), "pesticide", ifelse(BBS_ln_scale$COMPOUND %in% c("ACETAMIPRID", "CLOTHIANIDIN", "FIPRONIL", "IMIDACLOPRID", "THIACLOPRID", "THIAMETHOXAM"), "neonic", "ERROR"))
BBS_ln_scale$DIET_VULN <- ifelse(BBS_ln$DIET %in% c("insects", "seeds", 'nectar', "birds"), "diet_vuln", "diet_invuln")
BBS_ln_scale$DIET_VULN <- factor(BBS_ln$DIET_VULN)

BBS_ln_scale$DIET_INSECTS <- ifelse(BBS_ln$DIET == "insects", "diet_insects", "diet_not_insects")
BBS_ln_scale$DIET_INSECTS <- factor(BBS_ln$DIET_INSECTS)

names(BBS_ln_scale)
BBS_ln_scale2 <- BBS_ln_scale %>%
  ungroup() %>%
  group_by(YEAR, RTENO, BBS_ROUTE, BUFFER, AOU, 
  ...
  dplyr::summarise(sum_abundance = sum(SUM_ROUTE_ABUNDANCE),
            sum_KG_HIGH = sum(HIGH_KG_BUFF))
  
BBS_ln_scale2 <- BBS_ln_scale2 %>% 
  ungroup() %>%
  group_by(NEONIC, BUFFER) %>%
  mutate(scale_scale_ = as.numeric(scale(sum_KG_HIGH))) %>%
  ungroup() %>%
  group_by(BUFFER) %>%
...
class(BBS_ln_scale2$BBS_ROUTE)
BBS_ln_scale2$ROUTE_FAC<- as.factor(BBS_ln_scale2$BBS_ROUTE)
